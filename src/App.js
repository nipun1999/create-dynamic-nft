...
const updateExistingNFT = async () => {
  try{

    const img = await convertDOMtoBase64('updateImage');
    const templateImage = dataURLtoFile(img, 'My_NFT_2.png');
    const ownerPublicKey = new PublicKey(provider.publicKey).toBase58();
    ...
    await updateNFT(connection,provider,{},[templateImage],metadata,mintKey);
  }catch(err){
    console.log(err)
  }
}
  ...
  return (
    <div className="App">
      <header className="App-header">
           ...
           <img src={updateImage} style={{width:"200px"}} id="updateImage"></img>
          ...
          />
          <button onClick={updateExistingNFT}> { 'Update NFT' } </button>
